...
|-------------|------|-------------|--------|
| 电路外设 | `embedded-frontend/src/peripherals/` | `deviceCatalog.listDevices()` | 资产库、Manifest→画布、binding 校验 |
| 开发板 | `embedded-frontend/src/boards/` | `deviceCatalog.getBoard()` / `listBoards()` | 引脚解析、Board 画布布局 |
| 机械/环境 | `embedded-frontend/src/world-assets/` | `listMechanicalModels()` / `listEnvironmentModels()` | Bindings 面板、模板 |
