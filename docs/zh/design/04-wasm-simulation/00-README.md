...
> **现行入口**：3.0 已于 2026-08-02 通过 §7 门禁切换为 **Active**，是 Wasm 仿真设计的现行 SSOT 阅读入口。2026-08-11 依据 `review.md` 评审补充 **Amend**：修正 PWM 通道分类（通道 1b）、声明“Behavioral 降级不得破坏脉宽/测距信息语义”、补充控制面三线（IRQ/DMA/Timer）。TS 侧配套设计见 `unisim/docs/design/unified-peripheral-channel-architecture.md`。
