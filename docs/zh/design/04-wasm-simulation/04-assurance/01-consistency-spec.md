...
- **保障方案**：**B** 定义步内顺序"先读控制 → 算 plant → 再注入"；测试固定该序。**激励源二选一互斥**：同一场景内"脚本固定输入"与"Plant 自激演算"不得混用（[ADR-0067](../../../decisions/unisim/0067-appliance-plant-profile-architecture.md) §1）。脚本临时覆写 ADC 码值（冷水注入/探头扰动）属物理环境激励，释放后 Plant 必须基于**当前实测温度平滑恢复微分求解**，严禁滞后冲击鬼影（[ADR-0069](../../../decisions/unisim/0069-appliance-cross-category-safety-extension.md) §3）。
