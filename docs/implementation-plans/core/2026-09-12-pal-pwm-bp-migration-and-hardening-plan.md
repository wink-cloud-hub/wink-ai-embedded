...
| P3 | wasm ABI 断链 | `js_pal_pwm_set_duty_bp` 已声明（`wasm_bridge.h:95`）且 JS shim 存在（`wink_sim_js.js:107`），但**无 C 调用者、TS `WasmImports` 未声明**（`unisim/types/wasm/imports.ts:19` 仅 float）；unisim ABI catalog 已标 needs-fix |
...
- **T5.1** `unisim/types/wasm/imports.ts:19` 邻域新增 `js_pal_pwm_set_duty_bp(channel: number, basisPoints: number): void`（注释范围 `[0,10000]`，ADR-0066）。
- **T5.2** `unisim/core/bridge/bridge-factory.ts:124,258`：实现 bp 导入并转百分比后喂既有 `pwmSink`（`pluginHost.notifyDutyChange` 契约不变）。
- **T5.3** `embedded-frontend/simulation-kernel/workers/sim-worker.ts:225`：类型/注释核对（pwmSink 保持 `duty: percent`）。
...
| wink-ai | `unisim/types/wasm/imports.ts` | 改 | W-5 |
| wink-ai | `unisim/core/bridge/bridge-factory.ts` | 改 | W-5 |
| wink-ai | `embedded-frontend/simulation-kernel/workers/sim-worker.ts` | 核对/注释 | W-5 |
