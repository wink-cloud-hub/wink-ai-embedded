...
**Tech Stack:** Vue 3 + TypeScript + Vite、Vitest、Emscripten wasm32、现有 `sim-worker.ts` + `@unisim/worker`。
...
- 单测：`cd embedded-frontend && bun run test` 全绿为每 Task 门禁。
...
│ embedded-frontend/src/services/sim-runtime.ts       ✏️        │
...
│ embedded-frontend/src/components/workbench/ActuatorPanel.vue 🆕   │
└──────────────────────────────────────────────────────────────────────┘
```
...
| 文件 | 变更 | Task |
|------|------|------|
| `embedded-frontend/scripts/build-wasm.mjs` | ✏️ 解析 `wink-micro-app/` | T1 |
| `embedded-frontend/package.json` | ✏️（可选）文档化 app 路径 | T1 |
| `embedded-frontend/README.md` | ✏️ 避障 demo 步骤 + 舵机可见说明 | T7 |
| `wink-micro-os/CMakeLists.txt` | ✏️ 默认 `WINK_APP_DIR` 指向 micro-app（可选，与 T1 二选一） | T1 |
| `embedded-frontend/src/peripherals/servo/**` | 🆕 外设插件 + converter 注册 | T2 |
| `embedded-frontend/src/peripherals/index.ts` | ✏️ import servo | T2 |
...
| `embedded-frontend/src/types/actuator-observation.ts` | 🆕 语义 + Raw SSOT | T3 |
| `embedded-frontend/src/services/actuator-mapper.ts` | 🆕 Raw→Observation | T3 |
| `embedded-frontend/src/services/actuator-converter-registry.ts` | 🆕 converter registry（从 mapper 拆出，避循环依赖） | T3 |
| `embedded-frontend/src/peripherals/observe-builder.ts` | ✏️ `watchActuatorSource` | T3 |
| `embedded-frontend/src/types/sim-worker-protocol.ts` | ✏️ `ActuatorOutputBatch` + `actuatorSources` | T3 |
| `embedded-frontend/src/workers/sim-worker.ts` | ✏️ 采集 Raw batch + OBSERVE_PINS | T3 |
| `embedded-frontend/src/services/sim-runtime.ts` | ✏️ `actuatorObservations` + `lastActuatorSources` | T3 |
| `embedded-frontend/src/services/sim-client.ts` | ✏️ observe 缓存 + STATE_UPDATE mapper | T3 |
| `embedded-frontend/src/services/templates/avoidance-car-w2-minimal.ts` | ✏️ +neck_servo + props | T4 |
| `embedded-frontend/src/components/workbench/ActuatorPanel.vue` | 🆕 只读舵机角度 | T5 |
| `embedded-frontend/src/views/EmbeddedWorkbench.vue` | ✏️ 挂载面板 | T5 |
| `docs/design/.../04-phase-w3b-physics-actuators.md` | ✏️ §2.4 Spike 结论回写 | T7 |
| `embedded-frontend/src/**/__tests__/**` | 🆕/✏️ 单测 | T2–T6 |
...
**Modify:** `embedded-frontend/scripts/build-wasm.mjs`
...
- [ ] **Step 3:** 确认 `embedded-frontend/public/wasm/wasm-app-id.txt` 内容为 `avoidance_car`
...
**Create:** `embedded-frontend/src/peripherals/servo/`
...
**Create:** `embedded-frontend/src/types/actuator-observation.ts`（见 §2.5）
...
**Modify:** `sim-client.ts` — `observePins()` 在 postMessage 前将 `builder.build().actuatorSources` 写入 `runtime.lastActuatorSources`

#### 3.3 Mapper（★ 统一入口）🆕

**Create:** `actuator-mapper.ts`
...
**Modify:** `sim-client.ts` — `STATE_UPDATE` handler：
...
> **禁止**在 `applyStateUpdate()`（`sim-runtime.ts`）内调 mapper，保持数据面薄层。
...
**Create:** `components/workbench/ActuatorPanel.vue`
...
```typescript
const src = readFileSync('ActuatorPanel.vue', 'utf8');
...
**Modify:** `embedded-frontend/README.md` § Avoidance Car Demo
