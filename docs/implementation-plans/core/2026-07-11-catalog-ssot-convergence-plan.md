...
> **For agentic workers:** REQUIRED SUB-SKILL: Use `subagent-driven-development` or inline phase execution. Steps use checkbox (`- [ ]`) syntax. Each Phase ends with `npm test` + `npm run typecheck` + `npm run build` in `embedded-frontend/`.
...
**Tech Stack:** Vue 3.5 + Vite + TypeScript + Vitest；仅 `embedded-frontend/` 变更。
...
**Modify:** `embedded-frontend/src/peripherals/types.ts`
...
**Create:** `embedded-frontend/src/catalog/derive-catalog-entry.ts`
...
- [ ] A1: `grep -r "catalog\.pins" embedded-frontend/src/peripherals` 为 0
- [ ] A2: `grep -r "simulation\.worldCoupling" embedded-frontend/src/peripherals` 为 0
...
**Modify:** `embedded-frontend/src/catalog/device-catalog.ts`
...
```
embedded-frontend/src/world-assets/
  types.ts
  registry.ts
...
**Create:** `embedded-frontend/src/boards/esp32-devkit-v1/definition.ts` + `boards/registry.ts`

- 合并 `BOARDS` 与 `STATIC_DEVICES` 中的 board 条目
- `binding-pin-resolver` 继续通过 `deviceCatalog.getBoard()` 查询

**Delete:** `embedded-frontend/src/peripherals/legacy-adapter.ts`（及仅测 adapter 的用例，改测 registry 直连）
...
**Create:** `embedded-frontend/src/catalog/__tests__/catalog-mapping-consistency.test.ts`
