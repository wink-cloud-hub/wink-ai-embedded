...
| T4 ch3 模拟绑定 | ✅ **完成（活闭环 live-proven）** | C 侧 `js_pal_adc_read_norm(32+ch)`→12-bit 通；跨仓两缝已落地引擎侧：`afc54d68`（`js_pal_adc_read_norm` 从硬编 `return 0.0` 改接 `arbiter.readAnalog(pin)`，无驱动返 0 零回归）+ `81b94565`（headless 域上下文把共享 `PinArbiter` 传入 `AdcDomainHandler`——此前 headless ADC 写进断线 store）。headless 载体 `mcs51_analog_threshold`（embedded `5cf1c6d`）：`INPUT_ANALOG adcChannel:32` valueNorm 0.8→0.2→0.8 → CMS8S 片内 12-bit ADC → 固件阈值翻转 P1.0 LED，`ASSERT_POINT` 8/8。iron_ntc 插件路径同源（同 arbiter 模拟轨），未单独跑 |
