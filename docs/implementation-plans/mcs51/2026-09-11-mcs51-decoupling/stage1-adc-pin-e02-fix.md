...
- [ ] **目录与接口落位**：`mcs51_adc.cpp` 收 rail key 直透，无合成计算残留；`mcs51_adc0832.cpp` + shim 仅 key 显式化，无语义变更。
- [ ] **E-02 闭环自审**：养生壶冷启动场景室温阻值正常，数码管假短路告警 E-02 彻底消除。
- [ ] **iron_ntc 无回归**：ADC0832 路径（32+ch 板级 key）读数与基线一致，线上 pull 链路不断。
- [ ] **兼容双读验证**：片上旧用例以 32+ch 传 AN 时触发告警且计数正确自增；板级 32+ch 用例不告警。
- [ ] **双轨状态**：`core-tests` 与 `cms8s-tests` 均全绿。
