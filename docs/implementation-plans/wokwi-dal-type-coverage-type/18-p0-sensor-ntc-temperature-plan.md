...
| **LUT 生成脚本（跨仓）** | `wink-tools/tools/codegen/scripts/ntc_lut_gen.py`（CLI + 可 import 库，与 `list_drivers.py` 同级）；codegen 生成器集成位于 `wink-tools/tools/codegen/generators/` |
...
路径：`wink-tools/tools/codegen/scripts/ntc_lut_gen.py`（与 `list_drivers.py` 同级，属 codegen 工具链，非驱动运行时）。双重用途：
