...
│   │   └── schema.json
│   ├── gate_app_hardware_capacity.py / transpile_app_keil_c51.py  # 改为读 manifests；禁硬编码 MEM_LIMITS/cms8s 正则（CPL-16）
...
* **改动范围**：
  ...
  2. `gate_app_hardware_capacity.py`、`transpile_app_keil_c51.py` 改为由芯片描述清单（Manifest）驱动，移除 Python 脚本内的硬编码正则。
