...
> **责任领域**：`wink-tools`（工具链平台，位于兄弟仓 `wink-tools/`）+ `wink-micro-os`（嵌入式固件，本仓）

> **路径约定**：下文 `<wink-tools>/` 指 wink-tools 检出根（当前事实路径 `wink-tools/`，分发模式落地后可能为 site-packages 或 submodule）。本开源仓 `wink-ai-embedded/` 内 **不存在** `wink-tools/` 目录，所有工具链路径均为 `<wink-tools>/tools/...`。
...
- wink-tools 真实代码在兄弟仓 `wink-tools/`；根 `pyproject.toml` 仅含 `[tool.pyrefly]`，**无 `[project]`、无 `[project.scripts]`、无 CLI entry point**。
