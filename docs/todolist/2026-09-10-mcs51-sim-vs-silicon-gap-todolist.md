...
| **创建日期** | 2026-09-10 |
| **所属模块** | `wink-micro-os/frameworks/mcs51/`、`wink-micro-app/mcs51_*`、UniSim 无头执行链（sister repo `unisim`） |
...
| 编号 | 状态 | 优先级 | 后端归属（定义见[后端责任划分](./2026-09-10-mcs51-sim-backend-responsibility-classification.md)） | 标题 | 真机后果 | 影响 health_pot？ |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| [GAP-01](#gap-01p0shim-常量错误gpiop13_mux_rxd-0x02--原厂-0x03) | ✅ 完成 | **P0** | A-07 | shim 常量错误：`GPIO_P13_MUX_RXD` 0x02 ≠ 原厂 0x03 | 用该宏的 RXD 应用收不到串口 | 否（硬编码 0x03，仅 TX） |
| [GAP-02](#gap-02p0uart-tx-链路零校验) | ✅ 完成 | **P0** | A-01 + A-03 | UART TX 链路零校验（TR1/波特率/TXD mux/SCON 全不看） | 波特率错/无 TXD 复用 → 无输出或乱码，仿真照发 | 是（前提性风险，见 §5） |
| [GAP-03](#gap-03p0缺少-8051-工具链编译门禁) | 🔧 部分（Task 0 工具链落地，CI 接线待做） | **P0** | A-07 | 缺少 8051 工具链编译门禁（用户源码从未被 C51 编译器编译） | C90 方言/容量超限真机编译失败，仿真无感 | 是（未验证） |
...
| [GAP-22](#gap-22p1第三轮自查中断语义映射表的休眠错误种子) | ✅ 完成 | **P1** | A-06（含家族门控） | 中断语义映射表 3 处向量错（UART1/I2C/SPI 撞车）+ ADC 优先级 EIP 寄存器错 | 当前无模型 raise 故零信号，将来一做就跳错向量 | 否 |
| [GAP-23](#gap-23p1第三轮自查未建模-sfrxsfr-静默影子无-tripwire实测覆盖-77101-sfr93204-xsfr) | ✅ 完成 | **P1** | A-07 | 未建模 SFR/XSFR 静默落影子、无 unsupported 计数（SFR 77/101、XSFR 93/204） | 用 EPWM/I2C/SPI/ACMP/WUT/Flash 的固件仿真"正常"真机无功能 | 否（health_pot 只用已建模寄存器） |
| [GAP-24](#gap-24p2第三轮自查经典-51-movx-外部总线与-iap-非易失区未建模) | 🔧 部分（2026-09-11：at89 总线双向冲突计数 + IAP 显式 unsupported + 双构建单测全绿，host 零回归；wasm/runner 接线待办） | P2 | A-08 | 经典 51 的 MOVX 外部总线占用 P0/P2/P3.6/7；IAP 非易失区不持久 | 把 XBYTE 搬到 at89 carrier 会与 GPIO 冲突假通过 | 否（CMS8S 内部 XRAM） |
| [GAP-25](#gap-25p2第三轮自查合集模拟脚数字读sbuf-重写递归c51-库与栈深) | 🔧 部分（2026-09-11：模拟脚屏蔽随 GAP-08 落地；SBUF 复写计数 + `MCS51-RECURSION` lint + sdcc 栈底线 + 红线 L15/库表落地，双构建全绿；Keil map 侧待 Tier-K） | P2 | A-05 + A-08 + B-04 | 模拟脚仍可数字读；SBUF 发送中重写不报错；递归仿真安全真机踩 overlay；Keil 库/栈面未验证 | 多类 AI 生成代码假通过 | 否（已人工核对） |
...
>
> **状态图例**：✅ 完成（代码+测试+wasm 回归）｜🔧 部分（工具/部分子项落地，剩余在状态列注明）｜⬜ 未开始。**编号账：GAP-01~25 共 25 个号，GAP-16 并入 GAP-13、GAP-18 并入 GAP-12、GAP-17' 为 GAP-17 修正版，故独立问题为 23 个。** 完成明细与验证证据见 §10 执行记录。
...
- [x] 新增单元测试：VSEL=3V 与 Vrail=VDD(3.3V) 配置下同一 ratio 产生不同 raw，且与手算一致。（2026-09-11 `test_mcs51_adc_refchain`：0.5 norm → 2048@3.0V / 2252@3.3V）
- [x] LDO 未使能时启动转换触发 STRICT 断言。（Release 计数已验证；STRICT `assert+abort` 与 uart 同模式，strict 兼容库编译通过）
- [ ] health_pot device-tree 含外电路声明字段，缺字段时构建告警。（deferred 至阶段 3 Task 5）
...
- [x] TRIS=输入时写锁存器不产生对外电平通知；UP=1 的 HiZ 输入读回 1。（2026-09-11 `test_mcs51_gpio_dir`，host 通知日志证伪）
- [ ] 删掉 health_pot main 中的 P0UP 配置后，仿真出现显式告警（演示防线有效）后恢复。（测试级等价演示已过；真应用删除演示待 wasm 回归时补）
