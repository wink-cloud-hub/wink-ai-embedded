...
3. **Contracts as Single Source of Truth**: Interactions between repositories rely entirely on machine-readable contract specifications (`wink-app.json` Schema, `SimTraceSpec`, `wasm_bridge.h` ABI). As long as interface contracts remain stable, both repositories evolve independently.
...
└── unisim                                  # Cross-Repo Pillar 2: UniSim Wasm Simulation Engine (Black-box package)
    └── Interface Contracts: wasm_bridge.h C-ABI, SimTraceSpec (traceVersion: 1)
...
`wink-app.json` is the sole project definition document exchanged among `embedded-frontend`, `wink-tools`, and `wink-micro-os` (governed by the Day-0 V1.0.0 baseline and ADR-0073/ADR-0077):
...
- **`embedded-frontend` Responsibility**: Handles visual topology editing, property configuration, and serialization to `wink-app.json` (Workbench Manifest unified to `"schemaVersion": "1.0.0"`, absorbing legacy schema versions via tolerant migrations).
- **`wink-tools` Responsibility**: Parses `wink-app.json` and invokes `winkcli` to generate `device_tree.c` and related code (usage docs SSOT: [wink-tools/docs/](../../../../wink-tools/docs/en/01-cli-reference.md); enforcing top-level `schema_version == "1.0.0"` validation gate).
...
### 3.3 Contract 3: Simulation & Hardware Event Tracing Protocol (`SimTraceSpec`)

Used for Headless automated testing, frontend timeline visualization, and virtual-physical consistency comparisons:

- **Format**: JSONL / JSON structured envelope (`traceVersion: 1`) containing microsecond timestamps, device IDs, event types (`GPIO_SET` / `I2C_TRANSFER` / `FAULT_INJECT`), and state payloads.
- **Consumers**: Displayed in the `embedded-frontend` Trace Console and asserted in CI via `winkcli test`.

---

## 4. Dependency Security & Quality Gates

1. **Unidirectional Dependency**: The `wink-micro-os` C kernel strictly depends on zero Node.js/TS packages; `unisim` and `embedded-frontend` depend solely on the compiled Wasm binaries and public C struct headers exported by `wink-micro-os`.
2. **Build Isolation**: `winkcli build` executes in isolated sandbox containers; compilation scripts must never directly invoke private master-repo APIs.
3. **Version Locking**: Manifest `schema_version` and `SimTraceSpec` must enforce backward compatibility and migration testing across version bumps (`manifest-migration.ts`).
