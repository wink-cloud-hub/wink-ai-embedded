...
| T3 | Mapper 三输入 | `services/actuator-mapper.ts` | ✅ | 含 error handling |
| T3 | observe-builder 扩展 | `peripherals/observe-builder.ts:15,52-54` | ✅ | `watchActuatorSource` |
| T3 | Worker 采集 | `workers/sim-worker.ts:276-296` | ✅ | 含 export 存在性检查 |
| T3 | 方案 A client mapper | `services/sim-client.ts:108-111` | ✅ | mapper 在 client 层 |
| T3 | runtime 数据面 | `services/sim-runtime.ts:25-26` | ✅ | `shallowRef` |
...
| T5 | SimActuatorPanel | `components/workbench/ActuatorPanel.vue` | ✅ | 只读 `actuatorObservations` |
...
1. **`actuator-converter-registry.ts` 独立文件** — 计划中将 converter registry 定义在 `actuator-mapper.ts` 内。实现将其拆为独立文件，避免了 `servo/index.ts` import mapper 时产生的循环依赖（servo → mapper → peripherals/registry → servo）。这是正确的工程决策。
...
**文件：** `embedded-frontend/src/peripherals/servo/index.ts:8-23`
...
**文件：** `embedded-frontend/src/services/sim-runtime.ts:25-26`, `sim-client.ts:212`
...
**建议：** 确认模板切换时 `observePins()` 是否必然重新调用。如果是，则无需修改；如果不是，在 `sim-runtime.ts` 的 reset 逻辑中同步清空 `lastActuatorSources`。

### 4.3 [P2 — 建议补强] 测试覆盖不足

**文件：** `embedded-frontend/src/services/__tests__/actuator-observation.mapper.test.ts`
...
**文件：** `embedded-frontend/src/types/actuator-observation.ts:18`
...
**建议：** Phase 2 在 Mapper 或 `sim-client.ts` 的 STATE_UPDATE handler 中加 monotonically-ascending guard：
...
### 4.6 [P3 — 可选] `ActuatorPanel.vue` 显示精度

**文件：** `embedded-frontend/src/components/workbench/ActuatorPanel.vue:36`
