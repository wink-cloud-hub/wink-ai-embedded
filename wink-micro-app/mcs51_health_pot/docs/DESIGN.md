...
| 功能 | 说明 |
|---|---|
| **烧水模式（HEAT）** | 支持 **100 ℃ 沸腾模式**（`MODE_BOIL_100`，98 ℃ 沸腾确认 3 s 后转入保温）与 **55 ℃ 直热模式**（`MODE_DIRECT_55`，纯水/即饮模式，加热至 55 ℃ 立即断电转入保温）。加热过程中**允许按 FUNC 键动态变轨**：向上切换至 100 ℃ 继续沸腾；向下切换至 55 ℃ 时若水温已超标则立即切断发热盘转入保温；数码管实时显示水温与目标标识（100 ℃ 模式显示 `"XXb0"`，55 ℃ 模式显示 `"XX55"`，小数点仅在继电器实际吸合期间以 1 Hz 呼吸闪烁）；沸腾/到温完成播放 4 音和弦旋律，平滑转入保温 |
...
| **关机待机（OFF）** | 任意工作状态按 ON/OFF 键安全关断发热盘，回到待机状态；数码管默认显示 `" -- "`（若预选 55 ℃ 则显示 `"-55-"`），外设进入待机节能节拍。**待机时按 FUNC 可预选加热模式**。**传感器故障（E-01/E-02）下 ON/OFF 同样有效**：人工确认后进入「静音待机」（显示 `" -- "`、停止声光报警、继电器保持断开，故障码仍经遥测 F 字段上报），探头未恢复时再次按 ON/OFF 开机会被**拒绝并重新弹出故障**，读数连续 300 ms 有效后自动清除为普通待机 |
| **按键交互与消抖** | ON/OFF（P0.4）、FUNC（P0.5），20 ms 软件状态机消抖；按键触发硬件蜂鸣器 4 kHz 30 ms 机械按键触觉声效；功能切换触发 2 kHz → 3 kHz 阶梯升调反馈；待机与加热中按 FUNC 支持模式预选与动态变轨；任意时刻按 ON/OFF 一键急停关机，彻底消除关机安全盲区 |
