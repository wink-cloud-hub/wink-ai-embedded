...
| **五级安全故障防护** | ① **NTC 开路**（E-01）：12-bit 原始码 ≥ 3900（分压轨拉至满量程）；连续 3 次单向正跳变（加冷水/探头微动）同样报 E-01 开路类故障<br>② **NTC 短路/脱落**（E-02）：12-bit 原始码 ≤ 8（约 ≥120 ℃ 的零码带）；任何运行态下短路即切断，恢复目标为 OFF（**绝不自动重启加热**）<br>③ **干烧斜率保护**（E-03）：8051 定点 ADC 码域算法——16 s 环形窗（`adc_hist_16s[16]`）温升不足 6 码、且总加热 ≥20 s、有效纯净样本 ≥16 s、水温 <45 ℃ 时触发；单向加冷水上升沿（>40 码）复位历史环并重关斜率门（`slope_valid_sec`），下降沿升温自然穿透<br>③' **干烧超时兜底**（E-03，单镜像硬常数）：一级 650 s（<45 ℃）/ 二级 550 s（BOIL 档未达 98 ℃），覆盖敞盖散热、低电压、半壶水等越过 45 ℃ 后停滞工况；外部场景脚本严禁篡改<br>④ **超温保护**（E-04）：连续 10 s 原始码 ≤ 16（水温 > 105 ℃）；越界码期间**强制断开继电器且不允许完成沸腾确认**，杜绝"超温假沸腾"<br>**热故障 Sticky 锁存机制**：热故障（E-03/E-04）一旦锁存优先级最高，后续任何读数（含探头短路/开路）都不得降级或自清除，必须人工按 ON/OFF 键解除；同类传感器故障码允许实时刷新（开路↔短路）；探头故障（E-01/E-02）可由人工 ON/OFF 确认进入静音待机，或在读数连续 300 ms 有效后自动恢复至 OFF 态 |
