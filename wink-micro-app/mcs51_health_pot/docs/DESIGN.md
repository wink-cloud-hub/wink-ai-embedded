...
| 功能 | 说明 |
|---|---|
| **烧水模式（HEAT）** | 开机后全功率加热；水温达到 98 ℃ 判定沸腾；进入沸腾确认后**纯计时 3 s**（确认期内发热盘断开，水温因热惯性/NTC 滞后回落不清零、不重热，避免继电器在 98 ℃ 临界点频繁跳火振荡）；数码管实时显示水温与沸腾标识 `"XXb0"`（小数点仅在继电器实际吸合期间以 1 Hz 呼吸闪烁；3 s 沸腾确认期发热盘断开，dp 同步熄灭）；沸腾完成播放 4 音和弦旋律，随后平滑转入保温 |
...
| **关机待机（OFF）** | 任意工作状态按 ON/OFF 键安全关断发热盘，回到待机状态；数码管显示 `" -- "`，外设进入待机节能节拍。**传感器故障（E-01/E-02）下 ON/OFF 同样有效**：人工确认后进入「静音待机」（显示 `" -- "`、停止声光报警、继电器保持断开，故障码仍经遥测 F 字段上报），探头未恢复时再次按 ON/OFF 开机会被**拒绝并重新弹出故障**，读数连续 300 ms 有效后自动清除为普通待机 |
| **按键交互与消抖** | ON/OFF（P0.4）、FUNC（P0.5），20 ms 软件状态机消抖；按键触发硬件蜂鸣器 4 kHz 30 ms 机械按键触觉声效；功能切换触发 2 kHz → 3 kHz 阶梯升调反馈；FUNC 在待机/加热态无功能时给出 ~800 Hz 30 ms 短促否定音，避免"按键无反应"的死机错觉 |
...
| **HEAT（加热中）** | 1（**任何再吸合均受 3 s Dwell 门控**） | `"XXb0"`（dp 1Hz 闪烁） | **常亮** | 灭 | 灭 | 按键 4 kHz 30 ms；FUNC 无效时 ~800 Hz 否定音 |
...
| **看门狗（WDT）** | cms8s_sys 对 TA/CLKDIV/WDCON 做保护语义建模（未走 TA 解锁的写入被回滚），但不模拟超时复位 | 片内 WDT：`CKCON.WTS=0x06` 选择 2^24 Tsys ≈ 0.70s 溢出（24MHz）；WDTRE 使能与 WDTCLR 喂狗均须 TA 保护时序（`TA=0xAA; TA=0x55; WDCON`） | 主循环每 10ms 喂狗；0.70s 超时须大于最长阻塞段（遥测 TX 约 23ms）；后续可在仿真中补 WDT 超时复位模型 |
