...
| **保温模式（WARM）** | 目标温度 55 / 60 / 80 / 90 ℃ 四档，按 FUNC 键循环切换；采用 **±2 ℃ 迟滞 bang-bang 控温**（`WARM_HYST_C = 2`，保护继电器触点寿命）；数码管显示 `"XXYY"`（前两位实时水温，后两位设定水温） |
...
| **按键交互与极速响应** | ON/OFF（P0.4）、FUNC（P0.5），**10 ms Tick 守卫内双向 20 ms 去抖架构（20~30 ms 确认延迟）**：每一拍精确对应 10 ms 物理时基，连续 2 拍同值才确认按下/释放；上电 POST 卡键抑制（上电已按住的键不产生 phantom 事件，需完整释放+按下才响应）；待机与加热中按 FUNC 支持 55℃->80℃->100℃ 循环变轨；任意时刻按 ON/OFF 一键急停关机 |
...
| **4COM-8SEG 数码管显示** | 利用 CMS8S78xx 强驱动口（COM0..COM3 在 P3.0..P3.3 灌电流，SEG a..dp 在 P1.0..P1.7 推挽）实现 4 位 8 段共阴数码管分时动态扫描（Timer0 10 ms ISR 轮巡）：<br>• 待机态：`" -- "`<br>• 加热态：`"25b0"`（水温 25 ℃，b0 沸腾加热，dp 点 1 Hz 闪烁）<br>• 保温态：`"9860"`（水温 98 ℃，目标设定 60 ℃）<br>• 故障态：国际家电标准故障码 `"E-01"`（开路）、`"E-02"`（短路）、`"E-03"`（干烧）、`"E-04"`（超温）<br>• **冷却锁定态**：`"C00L"`（字模 'C'/'L' 新增，'O' 与数字 0 共用 `0x3F` 段码）与 `"-XX-"` 实测水温按 1 s 双相交替，直观告知用户系统在主动安全散热而非死机 |
...
| **六级安全故障防护** | ① **NTC 开路**（E-01）：12-bit 原始码 ≥ 3900（分压轨拉至满量程）；连续 3 次单向正跳变（加冷水/探头微动）同样报 E-01 开路类故障<br>② **NTC 短路/脱落**（E-02）：12-bit 原始码 ≤ 8（约 ≥120 ℃ 的零码带）；任何运行态下短路即切断，恢复目标为 OFF（**绝不自动重启加热**）<br>③ **干烧斜率保护**（E-03）：8051 定点 ADC 码域算法——16 s 环形窗（`adc_hist_16s[16]`）温升不足 6 码、且总加热 ≥20 s、有效纯净样本 ≥16 s、水温 <45 ℃ 时触发；单向加冷水上升沿（>40 码）复位历史环并重关斜率门（`slope_valid_sec`），下降沿升温自然穿透<br>③' **干烧超时兜底**（E-03，单镜像硬常数）：一级 650 s（<45 ℃）/ 二级 550 s（BOIL 档未达 98 ℃），覆盖敞盖散热、低电压、半壶水等越过 45 ℃ 后停滞工况；外部场景脚本严禁篡改<br>④ **超温保护**（E-04）：连续 10 s 原始码 ≤ 16（水温 > 105 ℃）；越界码期间**强制断开继电器且不允许完成沸腾确认**，杜绝"超温假沸腾"<br>⑤ **热余温冷却锁定**（E-03/E-04 后 60 s，GB 4706.1 19 章）：`cooldown_seconds` RAM 单字节在秒任务顶层无条件递减，E-03/E-04 重入刷新 60 s；锁定期 OFF→HEAT / WARM 重煮两条入口均拒绝并播否定音，主循环 `P2.0` 输出级再作终极硬钳位；**上电时序链**在首次 tick 前同步采样探头，≥45 ℃ 即强制续锁 60 s，彻底封堵"拔插头重开热盘"漏洞<br>**热故障 Sticky 锁存机制**：热故障（E-03/E-04）一旦锁存优先级最高，后续任何读数（含探头短路/开路）都不得降级或自清除，必须人工按 ON/OFF 键解除；同类传感器故障码允许实时刷新（开路↔短路）；探头故障（E-01/E-02）可由人工 ON/OFF 确认进入静音待机，或在读数连续 300 ms 有效后自动恢复至 OFF 态 |
...
│ 2. 消影：P3 COM 全灭      │ ──置位──▶ │ 每 10 ms：按键双向去抖 button_scan_10ms()（Tick 守卫内）│
...
stateDiagram-v2
    ...
    note right of FAULT
        ...
        E-01: NTC 探头开路 / 脱落
        E-02: NTC 探头短路 / 击穿
        E-03: 干烧报警（16 s 斜率窗 / 650 s 一级 / 550 s 二级）
        E-04: 超温报警 (水温越界失控)
...
| **COOL（余温冷却锁定，E-03/E-04 后或上电 ≥45 ℃）** | **0（输出级硬钳位）** | `"C00L"` / `"-XX-"`（1 s 交替） | 灭 | 灭 | 灭 | 尝试开机被拒时 ~800 Hz / 30 ms 否定音（TONE_BUSY） |
...
| `'C'` (索引 15) | a, d, e, f | `0x39` | `'L'` (索引 16) | d, e, f | `0x38` |
...
工程配套 26 个全自动化 Headless 验证场景（含 3 个 GB 4706 安规专项），涵盖快乐路径、边界保护、触点延寿、全状态电源安全、余温冷却锁定与外设交互：
...
| `health-pot-dryfire.scenario.json` | **干烧斜率 Sticky 锁存验证**：静态 25 ℃ 输入下第 20 个 tick 秒（场景约 21 s）斜率法触发 E-03 → 即使探头被干烧拖入短路区也不降级 → 探头恢复不自动清除 → 必须人工按键解除 | 🟢 100% PASS（热故障优先级锁定与手动清除） |
...
| `health-pot-dryfire-stall.scenario.json` | **二级干烧（沸腾超时 550 s）**：水温越过 45℃ 后停滞在 55℃（敞盖/低电压），斜率门因 ≥45 ℃ 不触发；持续加热 550 s 未沸腾 → E-03 锁存（场景 565 s 断言，含 ~1% tick 漂移裕量），手动解除 | 🟢 100% PASS（填补 45℃ 以上保护盲区） |
...
| `safety-cooldown-lock.scenario.json` | **60 s 余温冷却锁定（GB 4706.1 19 章）**：E-03 锁存 → 人工确认回 OFF → 锁定期内再按开机被拒（遥测 `S=0`、P2.0=0，显示 `"C00L"`/`"-25-"` 交替）→ ~81 s 到期后一次开机转入 HEAT 即收尾 | 🟢 100% PASS（先消费后拦截、无冷却到期幽灵自启动） |
| `safety-cold-water-injection.scenario.json` | **冷水注入不误报干烧（掩模规则）**：加热 5 s 后 +50 码冷水跳变重置斜率门，脚本按"任意 16 s 窗口降 ≥6 码"复温爬坡，持续加热 53 s 无 E-03 | 🟢 100% PASS（单向上升沿守卫 + 合法复温斜率） |
| `safety-relay-weld-protection.scenario.json` | **HIL 独占声明**：继电器触点烧结无自闭环感知（无电流互感器回路），焊接预言交 HIL（壶底双金属片 + 不可逆 TCO 硬件兜底）；仿真仅记录指令事件日志，`tags: ["HIL-Exclusive"]`，不作为仿真放行依据 | ⚪ HIL 独占声明 |

> **上电热态拦截（`safety-power-cycle-hot-reboot`）与 POST 卡键（`safety-post-jammed`）** 的 headless 场景依赖外仓 runner「中途复位 + t=0 初始条件」能力（D-005），暂缓落地避免假阳性；其核心断言（热态续锁、卡键抑制与恢复）已由 host 安规单测 `test_mcs51_health_pot_safety` 承载。
...
└── unisim-scenarios/
    ...
    ├── safety-cooldown-lock.scenario.json         # 60 s 余温冷却锁定（安规）
    ├── safety-cold-water-injection.scenario.json  # 冷水注入不误报干烧（安规）
    └── safety-relay-weld-protection.scenario.json # 继电器烧结 HIL 独占声明
```
...
| **干烧保护时间** | 斜率法（16 s 窗/6 码，20 s 武装）+ 单镜像硬兜底 650 s（< 45 ℃）/ 550 s（BOIL 未沸腾）；场景脚本只能配物理输入环境，严禁篡改安全硬常数 | 一级 120 ~ 180 s；二级沸腾超时 600 ~ 900 s（视水壶容积与功率而定） | 标称宽容限 650 s/550 s 已覆盖量产下限；真机按壶体标定微调并引入温升斜率 $\Delta T / \Delta t$ 动态算法 |
...
| **余温冷却锁定（60 s）** | `cooldown_seconds` RAM 单字节 + 上电同步采样（≥45 ℃ 强制续锁 60 s）；无 NVS，掉电即失但重上电立即重建 | 掉电期间靠壶体热容自然冷却；双金属片与不可逆 TCO 是最终硬件兜底（软件不可感知触点烧结） | 60 s 为标称安全值（按壶体热容实测只可延长不可缩短）；量产建议将故障历史/冷却余量写入 Data-Flash（P2 储备） |
