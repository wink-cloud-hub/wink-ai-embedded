# CMS8S78xx EXTINT1 运行、测试与可观测性指南

> **应用目录**：`wink-micro-app/vendor_cms8s78xx_v202_extint1`  
> **上游源码**：`docs/vendors/Cmsemicon/CMS8S78xx_DemoCode_V2.0.2/CMS8S78xx_Example/Example/EXTINT/EXTINT1/code`  
...
本程序是中微半导体官方 SDK 的 **EXTINT1（外部中断 1）** 基础例程。在保持官方源码“一行不改”的前提下，通过 WinkMicroOS 清洗与仿真底座运行。
...
* **状态指示引脚 (P3.2 / Pin 26)**：
  ...
  * 上电初始电平为 `0`（低电平）。
* **外部中断 1 触发引脚 (P3.1 / Pin 25)**：
  * 配置为数字输入模式 (`P31CFG = GPIO_MUX_GPIO`, `P3TRIS &= ~GPIO_PIN_1`)。
  * 开启片内弱上拉电阻 (`P3UP |= GPIO_PIN_1`)，悬空/未按下时引脚稳定保持为高电平 `1`。
  * **引脚复用选择器 (PS, Pin Selection)**：CMS8S78xx 支持引脚复用重定向，通过 `GPIO_SET_PS_MODE(PS_INT1, GPIO_P31_MUX_INT1)` 将物理引脚 P3.1 接通至内核的 INT1 中断检测线。

### 2. 中断机制与执行逻辑
1. **触发方式**：`EXTINT_ConfigInt(EXTINT1, EXTINT_TRIG_FALLING)`，设置为**仅下降沿触发**（Edge Triggered）。
2. **中断使能**：开启 INT1 中断 (`EXTINT_EnableInt(EXTINT1)`)，配置优先级为高 (`IRQ_PRIORITY_HIGH`)，并开启 CPU 全局中断 EA (`IRQ_ALL_ENABLE()`)。
3. **主循环**：`while(1) { ; }`，CPU 在主循环中空转等待中断。
4. **中断服务函数 (ISR)**：
   在 `isr.c` 中，外部中断 1 触发向量为 `INT1_VECTOR`（向量号 2）：
   ```c
   ...
   ```
   每当外部引脚 P3.1 检测到一个下降沿（电平从 1 跳变到 0），硬件中断触发，P3.2 输出电平翻转一次（0 -> 1，或 1 -> 0）。
...
### 1. 为什么官方源码属于 ⚡ Level 3？
在原厂裸机开发中，为了保持代码轻量且不占用串口或显示驱动，官方采用“**示波器 IO 打点法**”：
* 原厂代码只翻转一个引脚 `P32 = ~P32`。
* 既没有屏幕画面，也没有控制台文本打印。
* **痛点**：如果不借助示波器等物理仪器，人眼无法察觉程序是否正常响应；在软件仿真中若不做外设映射，仿真界面也是一片静默。

### 2. UniSim 仿真环境对 Level 3 的双重赋能
WinkMicroOS UniSim 通过两项技术打破 Level 3 的黑盒：

```
                    ┌────────────────────────────────────────────────────────┐
                    │            Level 3: 原始 IO 电平与中断打点              │
                    │   P3.1 (输入下降沿) ──> INT1 ISR ──> P3.2 (电平翻转)    │
                    └──────────────────────────┬─────────────────────────────┘
                                               │
                      ┌────────────────────────┴────────────────────────┐
                      ▼                                                 ▼
       ...
       • P3.1 (Pin 25) ──> 虚拟按键 (button)             • 13 步微秒级确定性时钟步进
       • P3.2 (Pin 26) ──> 虚拟状态灯 (led)               • 自动化注入电平与断言翻转状态
```

1. **虚拟外设拓扑映射（将 Level 3 升维转化为 Level 1 视觉交互）**：
   在 `wink-app.json` 中声明：
   * 输入端：将 P3.1 声明为 `button`（默认高电平，按下接地触发下降沿）。
   * 输出端：将 P3.2 声明为 `led`（高电平亮，低电平灭）。
   * **结果**：原本需要示波器抓波形的程序，在浏览器中变成了直观的“**点击虚拟按键，虚拟 LED 亮灭翻转**”。
2. **确定性虚拟逻辑分析仪（Headless 微秒级波形断言）**：
   在没有显示界面的 CI/CD 自动化流水线中，通过 `unisim-scenarios/extint1.scenario.json` 模拟逻辑分析仪的探头打点，实现无需人工介入的 100% 自动化测试。
...
python packages/wink-tools/wink.py build sim --app vendor_cms8s78xx_v202_extint1
```
> **检查输出**：执行后会在 `wink-micro-app/vendor_cms8s78xx_v202_extint1/unisim-assets/` 下生成：
...
```powershell
cd D:\workspaces\ai-coding\wink-ai\wink-ai
python packages/wink-tools/wink.py sim run --app vendor_cms8s78xx_v202_extint1
```
...
python packages/wink-tools/wink.py sim run `
  --app vendor_cms8s78xx_v202_extint1 `
  --mode headless `
  --scenarios ../wink-ai-embedded/wink-micro-app/vendor_cms8s78xx_v202_extint1/unisim-scenarios/extint1.scenario.json
```

#### 场景测试步骤深度解析（13 步时序表）
场景文件 [extint1.scenario.json](../unisim-scenarios/extint1.scenario.json) 模拟了完整的真实硬件电气测试行为：

| 步骤 | 时间点 (timeUs) | 操作类型 / 目标 | 预期值 / 动作 | 物理测试意义 |
| :---: | :---: | :--- | :---: | :--- |
| 1 | `50ms` | `ASSERT_POINT` -> `gpio:25` (P3.1) | `1` | **上电初始状态**：验证 P3.1 内部上拉生效，空闲时为高电平。 |
...
| 4 | `150ms` | `ASSERT_POINT` -> `gpio:25` (P3.1) | `0` | **输入波形捕获**：验证 P3.1 瞬间跳变至低电平，产生有效下降沿。 |
| 5 | `200ms` | `ASSERT_POINT` -> `gpio:26` (P3.2) | `1` | **首次中断响应**：验证 INT1 (向量2) 中断触发并执行 ISR，P3.2 翻转为 1 (LED 点亮)。 |
| 6 | `500ms` | `ASSERT_POINT` -> `gpio:25` (P3.1) | `0` | **长按保持态**：验证按键持续保持按下期间，P3.1 稳定锁死在 0。 |
...
| 9 | `1200ms` | `ASSERT_POINT` -> `gpio:25` (P3.1) | `1` | **释放波形捕获**：验证 P3.1 因内部上拉恢复为高电平，产生上升沿。 |
| 10 | `1200ms` | `ASSERT_POINT` -> `gpio:26` (P3.2) | `1` | **边沿单向性验证**：验证上升沿绝不触发 INT1，P3.2 依然保持为 1。 |
...
1. **外围连线**：
   * P3.1（Pin 25）接轻触按键一端，按键另一端接 GND；
