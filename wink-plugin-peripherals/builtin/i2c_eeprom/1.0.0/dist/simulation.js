...
var r = n(import.meta.url, "i2c_eeprom", "1.0.0", "generic"), i = 80, a = 32768, o = 64;
function s() {
	return t({
		type: r.type,
		version: r.version,
...
var c = s(), l = () => s(), u = class extends e {
	...
	_pageSize = o;
	_writeCycleUs = 0;
	_memory = new Uint8Array(a).fill(255);
	_pointer = 0;
	_writeAddress = 0;
	_addressPending = !1;
	_writeData = [];
	...
	onWriteByte(e, t) {
		return this.onAddressPhase(0) ? this._phase === "address" ? (this._addressBytes.push(e & 255), this._addressBytes.length === 2 && (this._writeAddress = ((this._addressBytes[0] << 8 | this._addressBytes[1]) >>> 0) % this._size, this._addressPending = !0, this._phase = "data"), !0) : (this._addressPending = !1, this._writeData.push(e & 255), !0) : !1;
	}
	onReadByte(e, t) {
	...
	onTransfer(e, t) {
		...
		let n = !0, r = 0, i = !1;
		for (let t = 0; t < e.length; t++) this.onWriteByte(e[t], t) || (n = !1, t < 32 ? r |= 1 << t : i = !0);
		let a;
		if (n && t > 0) {
	...
	deserializeState(e) {
		...
		if (typeof e.addressPointer == "number" && (this._pointer = e.addressPointer % this._size), typeof e.busyUntilUs == "string") try {
	...
	_finalizePending(e) {
		if (this._writeData.length > 0 && e) {
			let e = this._writeAddress - this._writeAddress % this._pageSize, t = this._writeAddress;
			for (let n of this._writeData) {
				let r = (t - e) % this._pageSize;
				this._memory[e + r] = n, t++;
			}
			if (this._pointer = t % this._size, this._writeCount++, this._addressPending = !1, this._addressBytes = [], this._writeData = [], this.ctx?.publish("addressPointer", this._pointer), this.ctx?.publish("writeCount", this._writeCount), this._writeCycleUs > 0 && this.ctx) {
				this._busyUntilUs = this.ctx.nowUs() + BigInt(this._writeCycleUs), this.ctx.publish("busy", 1);
		...
		this._addressPending && (this._pointer = this._writeAddress % this._size, this._addressPending = !1, this.ctx?.publish("addressPointer", this._pointer)), this._addressBytes = [], e && (this._writeData = []);
	}
...
function f(e) {
	...
	for (let n = 0; n < e.length; n += 3) {
		...
		t += d[r >> 2], t += d[(r & 3) << 4 | i >> 4], t += n + 1 < e.length ? d[(i & 15) << 2 | a >> 6] : "=", t += n + 2 < e.length ? d[a & 63] : "=";
	}
	return t;
}
function p(e) {
	...
	for (let e = 0; e < t.length; e += 4) {
		let a = d.indexOf(t[e]), o = d.indexOf(t[e + 1]), s = e + 2 < t.length ? d.indexOf(t[e + 2]) : -1, c = e + 3 < t.length ? d.indexOf(t[e + 3]) : -1;
...
//#endregion
export { u as I2cEepromPlugin, s as createI2cEepromManifest, m as default, c as i2cEepromManifest, l as i2cEepromManifestFactory };
