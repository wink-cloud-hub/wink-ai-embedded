# AGENTS.md — Wink-AI 嵌入式（WinkMicroOS）

> 本文件是 AI Coding Agents（包括 Claude Code、Antigravity 等）在本项目的必读指南，每次会话启动时加载。
...
- **分层门禁：`winkcli lint`（ADR-0043）**: App/BAL/DAL/PAL 边界与 API 形态由 `wink-tools/tools/lint/rules/*.yaml` 驱动；生成或修改 C 代码后应运行 `winkcli lint --pack layering --pack api`（安装与用法见 `wink-tools/README.md` 与 `wink-tools/docs/`）。
...
## Documentation
- 详细 C 编码与 API 规范见 [.agents/rules/c-code.md](file:///d:/workspaces/ai-coding/wink-ai/wink-ai-embedded/.agents/rules/c-code.md)。
- 详细文档库组织与 ADR 规范见 [.agents/rules/docs-adr.md](file:///d:/workspaces/ai-coding/wink-ai/wink-ai-embedded/.agents/rules/docs-adr.md)。
