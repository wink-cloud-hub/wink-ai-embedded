...
#include "pal_hal_esp32_internal.h" /* pal_esp32_gpio_synchronize_all() cross-TU declaration */
#include <stdarg.h>
#include <stdio.h>
...
/* SMP ISR 同步 count（GPIO：s_gpio_irq_in_flight 由 pal_esp32_gpio_synchronize_all()
 * 供 pal_irq_esp32.c 通过 pal_hal_esp32_internal.h 消费）。 */
static volatile uint32_t s_gpio_irq_in_flight[GPIO_NUM_MAX] = {0};

/* 供 pal_irq_esp32.c 的 pal_irq_synchronize(~0U) 全量等待 GPIO ISR 使用。
 * 保持 s_gpio_irq_in_flight[] file-scope 于 GPIO TU，不跨 TU 暴露数组。 */
