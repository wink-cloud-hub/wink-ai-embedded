...
wink_status_t pal_adc_init(pal_adc_channel_t ch, const pal_adc_config_t *cfg) {
    ...
    memset(&s_channels[ch].rc_ctx, 0, sizeof(wink_phys_rc_ctx_t));
...
wink_status_t pal_adc_read_raw(pal_adc_channel_t ch, uint16_t *out_raw) {
    ...
    if (s_channels[ch].last_raw > max_raw) s_channels[ch].last_raw = (uint16_t)max_raw;
    s_channels[ch].last_mv  = (uint16_t)(norm_filt * (float)s_channels[ch].full_scale_mv + 0.5f);
    if (s_channels[ch].last_mv > s_channels[ch].full_scale_mv) {
        s_channels[ch].last_mv = s_channels[ch].full_scale_mv;
    }
