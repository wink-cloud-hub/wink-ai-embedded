...
//
// Two-phase interrupt architecture (ADR-0078):
//   - Phase 1 (Raise): Peripheral models call mcs51_raise_irq(src) to set the
//     pending bit in s_pending_interrupts without recursion.
//   - Phase 2 (Dispatch): Rendezvous points (microstep, yield) arbitrate
//     pending interrupts using 8051 in-service priority masking and natural
//     vector scan order, then dispatch to the registered vector.
...
uint32_t s_pending_interrupts = 0;
uint8_t  s_in_service_prio_stack[4] = {0};
uint8_t  s_in_service_depth = 0;
bool     s_reti_suppress_one = false;
...
bool wink_mcs51_in_isr(void) { return s_in_service_depth > 0; }
...
uint8_t wink_mcs51_dispatch_vector(uint8_t vector_num) {
    ...
    if (s_in_service_depth == 0) {
        s_in_service_prio_stack[0] = 0;
        s_in_service_depth = 1;
...
void wink_mcs51_reset_irq_state(void) {
    ...
    s_in_service_depth = 0;
    s_reti_suppress_one = false;
    for (uint8_t i = 0; i < 4; ++i) {
        s_in_service_prio_stack[i] = 0;
    }
...
uint8_t wink_mcs51_get_in_service_depth(void) {
    return s_in_service_depth;
}

uint8_t wink_mcs51_get_in_service_prio(uint8_t depth_index) {
    if (depth_index < s_in_service_depth) {
        return s_in_service_prio_stack[depth_index];
    }
    return 0;
}
...
void mcs51_raise_irq(mcs51_irq_source_t src) {
    ...
    if (entry.prio_sfr != 0xFFu) {
        prio = (wink_mcs51_sfr_shadow[entry.prio_sfr] >> entry.prio_bit) & 1u;
    }
    WINK_TRACE_MCS51_IRQ(src, entry.vector, prio, wink_mcs51_virtual_us(), MCS51_IRQ_EVT_RAISE);

#if !WINK_MCS51_TWO_PHASE_IRQ
    // Escape hatch: single-phase synchronous call
    if (!s_interrupts_enabled) {
        return;
    }
    if ((wink_mcs51_sfr_shadow[SFR_IE] & (1u << IE_EA)) == 0) {
        return;
    }
    if (entry.ie_sfr != 0xFFu && (wink_mcs51_sfr_shadow[entry.ie_sfr] & (1u << entry.ie_bit)) == 0) {
        return;
    }
    if (entry.clear_mode == MCS51_IRQ_HW_AUTO_CLEAR && entry.flag_sfr != 0xFFu) {
        wink_mcs51_sfr_shadow[entry.flag_sfr] &= ~(1u << entry.flag_bit);
    }
    wink_mcs51_dispatch_vector(entry.vector);
#else
    if (entry.flag_sfr != 0xFFu && src != IRQ_SOURCE_UART0) {
        if (src == IRQ_SOURCE_INT0) {
            bool edge = (wink_mcs51_sfr_shadow[0x88] & (1u << 0)) != 0;
            if (edge) {
                wink_mcs51_sfr_shadow[entry.flag_sfr] |= (1u << entry.flag_bit);
            }
        } else if (src == IRQ_SOURCE_INT1) {
            bool edge = (wink_mcs51_sfr_shadow[0x88] & (1u << 2)) != 0;
            if (edge) {
                wink_mcs51_sfr_shadow[entry.flag_sfr] |= (1u << entry.flag_bit);
            }
        } else {
...
uint8_t mcs51_irq_scan_and_dispatch(void) {
    ...
    int current_prio = -1;
    if (s_in_service_depth > 0) {
        current_prio = (int)s_in_service_prio_stack[s_in_service_depth - 1];
    }
    ...
    for (uint8_t s = 0; s < IRQ_SOURCE__COUNT; ++s) {
        if ((s_pending_interrupts & (1u << s)) == 0) {
            continue;
        }
        ...
        if (entry.ie_sfr != 0xFFu) {
            if ((wink_mcs51_sfr_shadow[entry.ie_sfr] & (1u << entry.ie_bit)) == 0) {
                continue;
            }
        }
        ...
        if (entry.flag_sfr != 0xFFu) {
            ...
            if (s == IRQ_SOURCE_UART0 && entry.flag_sfr == SFR_SCON) {
                flag_set = (wink_mcs51_sfr_shadow[SFR_SCON] & 0x03u) != 0;
            } else if (s == IRQ_SOURCE_INT0) {
                bool edge = (wink_mcs51_sfr_shadow[0x88] & (1u << 0)) != 0;
                flag_set = !edge || ((wink_mcs51_sfr_shadow[0x88] & (1u << 1)) != 0);
            } else if (s == IRQ_SOURCE_INT1) {
                bool edge = (wink_mcs51_sfr_shadow[0x88] & (1u << 2)) != 0;
                flag_set = !edge || ((wink_mcs51_sfr_shadow[0x88] & (1u << 3)) != 0);
            } else {
                flag_set = (wink_mcs51_sfr_shadow[entry.flag_sfr] & (1u << entry.flag_bit)) != 0;
            }
            if (!flag_set) {
                s_pending_interrupts &= ~(1u << s);
                continue;
            }
        }
    ...
    s_pending_interrupts &= ~(1u << best_src);
    ...
    if (s_in_service_depth < 4) {
        s_in_service_prio_stack[s_in_service_depth++] = (uint8_t)best_prio;
    }
