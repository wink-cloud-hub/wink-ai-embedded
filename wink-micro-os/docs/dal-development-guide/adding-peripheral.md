...
与仿真侧 [ADDING_PERIPHERAL.md](file:///d:/workspaces/ai-coding/wink-ai/unisim/docs/ADDING_PERIPHERAL.md) 配套。设备 `type` 字符串**两侧必须完全一致**。
...
> **DAL vs Role（分清两步）**  
> 本指南覆盖 **DAL + codegen 插件注册**（`type`、引脚、CMake、`WINK_USE_*`）。  
> 把驱动包装成 App 的 `{name}_{verb}` 属于 **可选的 Role Interface（codegen）**，代码在 `wink-tools/tools/codegen/`，见专文 [role-interface-codegen.md](./role-interface-codegen.md)。  
> `new-dal` 会创建插件骨架，但**不要求**立刻实现 `role_verbs`；无 role 的驱动（如现网 `dc_motor`）完全合法。
...
**可选下一步**：在同一 `drivers/<type>.py` 中挂 Role → [role-interface-codegen.md](./role-interface-codegen.md)。
