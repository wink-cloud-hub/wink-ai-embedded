...
2. 新建 `codegen/drivers/<type>.yaml`，`type:` 字段**必须**与文件名 stem 一致；含 `codegen_schema: 1`、`config`、`constraints`、可选 `role_bindings`。  
3. 复杂 init 逻辑放 `codegen/drivers/templates/<type>_init.c.j2`，YAML 里 `config.init_template_file` 引用相对路径。  
4. 参考现网样例：`codegen/drivers/led.yaml`、`ultrasonic.yaml`、`ssd1306.yaml`。
