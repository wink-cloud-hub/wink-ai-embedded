...
App / BAL      ──调用──►  dal_<type>_*（命名 API，无虚表）
DAL            ──调用──►  pal_*（真机）或仿真旁路（SIMULATION）
```

- App **不**直接玩 GPIO/PWM 时序；只调 `dal_*`。
...
/* 开环：speed ∈ [-1.0, 1.0]；0 → coast */
WINK_IGNORE_UNUSED(dal_dc_motor_set_speed(&left_motor, 0.5f));
```
