...
| **Codegen 扩展根（ADR-0051）** | 各 `type` 的 codegen 描述与 Role bindings 外置在 [`wink-micro-os/codegen/`](../codegen/README.md)（**Schema 1.1**：单一 `fields:` SSOT），加外设无需改 `wink-tools` 源码 — 见 [adding-peripheral.md](./dal-development-guide/adding-peripheral.md) |
