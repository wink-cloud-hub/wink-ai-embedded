...
/**
 * @file pal_deferred_host.c
 * @brief Host implementation of PAL Deferred-Call Worker subsystem.
 */
...
static pal_deferred_host_queue_t s_queues[PAL_DEFERRED_PRI_COUNT];
...
void pal_deferred_host_drain(void) {
    if (!s_initialized) return;

    for (int p = 0; p < PAL_DEFERRED_PRI_COUNT; p++) {
        pal_deferred_host_queue_t *q = &s_queues[p];
        while (q->count > 0) {
...
static wink_status_t deferred_post_internal(pal_deferred_pri_t pri,
    ...
    pal_deferred_host_queue_t *q = &s_queues[pri];
    ...
    /* On Host target, automatically drain to execute in simulated tick */
    pal_deferred_host_drain();
