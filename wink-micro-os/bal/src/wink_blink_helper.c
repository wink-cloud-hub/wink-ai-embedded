 ...
 * (iterating the slot array is O(WINK_BLINK_HELPER_MAX) �?4, negligible).
 *
...
static blink_ctx_t s_slots[WINK_BLINK_HELPER_MAX];
...
static int find_free_slot(void) {
    for (int i = 0; i < WINK_BLINK_HELPER_MAX; i++) {
                                ...
                                const wink_bal_opts_t *opts)
{
    ...
    if (free_idx < 0) {
        LOG_D("start: out of blink slots (%d)", WINK_BLINK_HELPER_MAX);
