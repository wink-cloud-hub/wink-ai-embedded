...
> **构建目录与应用配置约定**：全量 host 套件需要完整驱动集（多 sample 共享同一 `dal`，ADR-0039），因此测试构建目录请使用非 mcs51 应用（默认 `avoidance_car` 或任意普通应用）。纯 mcs51 应用（如 `mcs51_health_pot`）会让根 CMake 在目录作用域关闭全部 DAL 驱动（ADR-0075），该目录仅适合 mcs51 专项构建；两个用途建议使用独立目录（如 `build/host` 与 `build/host-mcs51`）。切换应用后若发现驱动被历史缓存残留关闭，`cmake -U "WINK_USE_*"` 一次即可恢复默认。
