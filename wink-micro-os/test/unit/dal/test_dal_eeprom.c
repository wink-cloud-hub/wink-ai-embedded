...
void test_eeprom_init_claims_resource_and_deinit_releases(void) {
    dal_eeprom_t dev = {0};
...
void test_eeprom_rw_operations(void) {
    dal_eeprom_t dev = {0};
    const dal_eeprom_config_t cfg = {
        .i2c_port = 0, .i2c_addr = 0x50, .capacity_bytes = 256,
        .page_size = 8, .write_time_ms = 5, .owner = "eeprom0",
    ...
    uint8_t out[4] = {0};
    TEST_ASSERT_EQUAL_INT(WINK_OK, dal_eeprom_read_blocking(&dev, 0, out, 3));
...
int main(void) {
    ...
    RUN_TEST(test_eeprom_init_claims_resource_and_deinit_releases);
    RUN_TEST(test_eeprom_rw_operations);
    return UNITY_END();
}
