 ...
 * @brief Unit tests for BAL wink_sonar_helper (host).
 *
...
static dal_ultrasonic_t s_sonar1;
static dal_ultrasonic_t s_sonar2;
...
void setUp(void) {
    ...
    memset(&s_sonar1, 0, sizeof(s_sonar1));
    ...
    TEST_ASSERT_EQUAL_INT(WINK_OK, dal_ultrasonic_init(&s_sonar1, &cfg1));

    memset(&s_sonar2, 0, sizeof(s_sonar2));
    ...
    TEST_ASSERT_EQUAL_INT(WINK_OK, dal_ultrasonic_init(&s_sonar2, &cfg2));
}

void tearDown(void) {
    WINK_IGNORE_RESULT(dal_ultrasonic_deinit(&s_sonar1));
    WINK_IGNORE_RESULT(dal_ultrasonic_deinit(&s_sonar2));
    sim_clear_gpio_ideal();
}
...
void test_sonar_helper_lifecycle(void) {
    TEST_ASSERT_FALSE(wink_sonar_helper_is_running(&s_sonar1));
    TEST_ASSERT_EQUAL_INT(WINK_OK, wink_sonar_helper_start(&s_sonar1, 100));
    TEST_ASSERT_TRUE(wink_sonar_helper_is_running(&s_sonar1));
...
void test_sonar_helper_duplicate_start_rejected(void) {
    TEST_ASSERT_EQUAL_INT(WINK_OK, wink_sonar_helper_start(&s_sonar1, 100));
    TEST_ASSERT_EQUAL_INT(WINK_ERR_INVALID_STATE, wink_sonar_helper_start(&s_sonar1, 50));
    TEST_ASSERT_EQUAL_INT(WINK_OK, wink_sonar_helper_stop(&s_sonar1));
}

/* 4. Slot pool exhaustion */
void test_sonar_helper_pool_exhaustion(void) {
    /* If WINK_SONAR_HELPER_MAX is 4, we need 5 mock slots to exhaust the pool. */
    dal_ultrasonic_t mocks[WINK_SONAR_HELPER_MAX + 1];
    memset(mocks, 0, sizeof(mocks));

    int count = 0;
    for (int i = 0; i < WINK_SONAR_HELPER_MAX + 1; i++) {
        ...
        wink_status_t st = wink_sonar_helper_start(&mocks[i], 100);
        if (st == WINK_OK) {
            count++;
        } else {
    ...
    TEST_ASSERT_EQUAL_INT(WINK_SONAR_HELPER_MAX, count);

    // Clean up
    for (int i = 0; i < count; i++) {
        TEST_ASSERT_EQUAL_INT(WINK_OK, wink_sonar_helper_stop(&mocks[i]));
        TEST_ASSERT_EQUAL_INT(WINK_OK, dal_ultrasonic_deinit(&mocks[i]));
    }
}
...
void test_sonar_helper_set_period(void) {
    TEST_ASSERT_EQUAL_INT(WINK_ERR_INVALID_STATE, wink_sonar_helper_set_period(&s_sonar1, 200));

    TEST_ASSERT_EQUAL_INT(WINK_OK, wink_sonar_helper_start(&s_sonar1, 100));
    TEST_ASSERT_EQUAL_INT(WINK_OK, wink_sonar_helper_set_period(&s_sonar1, 200));
