 ...
 *     per test ù?setUp/tearDown guarantee a clean slot state between tests.
 ...
 *     BAL slot bookkeeping ù?which is exactly what that regression guards.
 *
 * Verifies:
 *   - NULL ultrasonic/btn are both valid (runtime-stats-only telemetry).
 *   - is_running() lifecycle.
 *   - Duplicate-start ù?WINK_ERR_INVALID_STATE.
...
/* REGRESSION for BAL slot/handle leak ù?100 LIGHT start/stop cycles.
...
/* _start_ex with explicit MAY_BLOCK opts (one start per test ù?host
