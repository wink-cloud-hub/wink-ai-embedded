...
### 前置条件
- 通过 Espressif IDE Manager (EIM) 安装 ESP-IDF v6.x —— 参见 [`wink-micro-os/tools/preinstall.md`](../wink-micro-os/tools/preinstall.md) §3。Wink 永远不会自动安装 IDF（ADR-0030）。
- PATH 上有 Python 3.10+。
...
仅在 `idf.py menuconfig`、`idf.py size`、`idf.py gdb`、`idf.py efuse-*` 等高级交互式工作流下。直接调用 `idf.py` 时必须先手工激活 IDF shell（source EIM PowerShell profile 或执行 `export.ps1`/`export.sh`），因为 Wink 不会替你激活。手工激活方式参见 [`preinstall.md §3`](../wink-micro-os/tools/preinstall.md)。
...
**不需要。** 扫描器会自动发现新文件，重新运行 `python wink-micro-os/tools/wink.py esp32 --app <name>` 即可。
...
运行 `python wink-micro-os/tools/wink.py doctor` 按提示排查；最常见原因是未通过 EIM 安装 ESP-IDF。参见 [`preinstall.md §3`](../wink-micro-os/tools/preinstall.md)。
...
构建工具位于 `../wink-micro-os/tools/esp32/`——`activate.py`（IDF 环境采集）、`build.py`（净化环境后调用 idf.py）、`generate_app_sources.py`（源文件扫描器）。**统一入口：`python wink-micro-os/tools/wink.py esp32`。** 详见 [`wink-micro-os/tools/README.md`](../wink-micro-os/tools/README.md)。
